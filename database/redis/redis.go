...
func InitRedis(configs RedisConfig) {
	if configs.Address == "" {
		panic("redis is nil")
	}
	c = configs
...
//func GetRedisConfig(c RedisConfig) RedisConfig {
//	var redisConfig RedisConfig
//	redisConfig.Address = configer.RedisConfig().Addr
//	redisConfig.Db, _ = strconv.Atoi(configer.RedisConfig().Db)
//	redisConfig.Password = configer.RedisConfig().Password
//	return redisConfig
//}
...
func (r *Redis) GetInt(key string) (int, error) {
	...
	case int64:
		...
		return x, nil
	case []byte:
		n, err := strconv.ParseInt(string(any), 10, 0)
		return int(n), err
	case nil:
		return 0, nil
	case redis.Error:
	...
	return 0, fmt.Errorf("redigo: unexpected type for Int, got type %T", any)
...
// Exists 检查键是否存在
func (r *Redis) Hexists(key, file string) (bool, error) {
	return redis.Bool(r.Do("HEXISTS", key, file))
}
...
func (r *Redis) Hdel(key, field string) (reply interface{}, err error) {
	reply, err = r.Do("HDEL", key, field)
	return
}
...
func (r *Redis) HgetFloat64(key, field string) (reply float64, err error) {
	reply, err = redis.Float64(r.Do("HGET", key, field))
	return
}
...
func (r *Redis) HincrbyInt(key, field string, value int) (reply int, err error) {
	reply, err = redis.Int(r.Do("HINCRBY", key, field, value))
	if err != nil {
		return reply, err
	}
	return reply, nil
...
func (r *Redis) HincrbyInt64(key, field string, value int64) (reply int64, err error) {
	reply, err = redis.Int64(r.Do("HINCRBY", key, field, value))
	if err != nil {
		return reply, err
	}
	return reply, nil
...
func (r *Redis) HincrbyFloat64(key, field string, value float64) (reply float64, err error) {
	reply, err = redis.Float64(r.Do("HINCRBYFLOAT", key, field, value))
	if err != nil {
		return reply, err
	}
	return reply, nil
...
// Sadd 将一个元素放入集合中去
//func (r *Redis) Sadd(key string, member []interface{}) (reply interface{}, err error) {
//	args := []interface{}{key}
...
// Sadd 将一个元素放入集合中去
func (r *Redis) Sadd(key string, member []string, expire int) (err error) {
	conn := r.pool.Get()
	defer conn.Close()
	err = conn.Send("SADD", redis.Args{}.Add(key).AddFlat(member)...)
	if err != nil {
		return
	}
	if expire > 0 {
...
// Srem 移除集合中元素， 不存在将被忽略
func (r *Redis) Srem(key string, member string) (reply interface{}, err error) {
	return r.Do("SREM", key, member)
}

// Sexit 判断元素是否在集合中
func (r *Redis) Sexit(key string, member string) (reply bool, err error) {
...
*/
// lpush
func (r *Redis) Lpush(key string, member []interface{}, expire int) (err error) {
	conn := r.pool.Get()
	defer conn.Close()
	err = conn.Send("LPUSH", redis.Args{}.Add(key).AddFlat(member)...)
	if err != nil {
		return
	}
	if expire > 0 {
...
//lpop
func (r *Redis) LpopFloat64(key string) (reply float64, err error) {
	return redis.Float64(r.Do("LPOP", key))
}

//llen
func (r *Redis) Llen(key string) (reply int, err error) {
